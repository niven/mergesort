...
void iterative_mergesort( void* base, size_t nel, size_t width, comparator compare ) {
	...
	char* mid;
	for( size_t merge_range = 2; merge_range < nel; merge_range *= 2 ) {

		say("Merging all chunks of size %zu\n", merge_range);
		
		for( from=base; from < to - 2*merge_range*width; from += 2*merge_range*width ) {
			to = from + 2*merge_range*width;
			mid = from + ((to-from) / (2*width))*width; // ugly
			say("start-mid-to = 0 - %d - %d\n", (mid-from)/width, (to-from)/width );
